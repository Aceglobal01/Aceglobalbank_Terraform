...
module "namecheap-deployment" {
    ...
    environment = var.environment
    domain-name = var.domain-name
    nginx_lb_ip                          = module.eks-deployment.nginx_lb_dns
    nginx_ingress_load_balancer_hostname = module.eks-deployment.nginx_lb_dns
    nginx_ingress_lb_dns                 = module.eks-deployment.nginx_lb_dns
